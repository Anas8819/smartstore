...
namespace SmartStore.Web.Controllers
{
    public partial class CustomerController : PublicControllerBase
    {
        ...
        //after filling data to register
        ...
        [HttpPost]
        public ActionResult PaymentRequest(PaymentRequestModel model)
        {
            PaymentRequest paymentRequest = new PaymentRequest();
            paymentRequest = ToEntity(model);
            var customer = _workContext.CurrentCustomer;
            paymentRequest.Customer = customer;
            paymentRequest.CustomerId = customer.Id;
            paymentRequest.CreatedOnUtc = DateTime.Now;
            paymentRequest.UpdatedOnUtc = DateTime.Now;
            paymentRequest.UserFullName = customer.GetFullName();
            paymentRequest.UserEmail = customer.FindEmail();
            paymentRequest.PaymentStatusId = 2;
            _paymentRequestService.InsertPaymentRequest(paymentRequest);
            return RedirectToAction("PaymentRequest");
        }

        private PaymentRequest ToEntity(PaymentRequestModel model)
        {
        ...
        private PaymentRequestModel ToModel(PaymentRequest entity)
        {
